...
func buildTeamMD(team *store.TeamData, members []store.TeamMemberData, selfID uuid.UUID) string {
	...
	if selfRole == store.TeamRoleLead {
		...
		sb.WriteString("- action=await_reply, task_id=<id>, followup_message=<msg> → set auto-reminder when waiting for user reply\n")
		sb.WriteString("- action=clear_followup, task_id=<id> → stop reminders after user replies\n\n")
		sb.WriteString("## Follow-up Reminders\n\n")
		sb.WriteString("When you ask the user a question and need their reply to proceed:\n")
		sb.WriteString("1. Create a task for the question (action=create)\n")
		sb.WriteString("2. Claim it (action=claim)\n")
		sb.WriteString("3. Set follow-up: action=await_reply, task_id=<id>, followup_message=\"<reminder text>\"\n")
		sb.WriteString("4. The system auto-sends reminders to the user's channel if they don't reply\n")
		sb.WriteString("5. When the user replies, call action=clear_followup to stop reminders\n")
		sb.WriteString("6. Process their answer and complete the task\n\n")
